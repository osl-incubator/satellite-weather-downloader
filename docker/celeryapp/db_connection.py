...
class BackfillDB:
                ...
                match table:
                    case 'brasil':
                        dates = pd.bdate_range(
                            start='1/1/2000', end='12/1/2022', freq='D'
                        )[::-1]
                        df = create_df(dates)
                    ...
                    case 'foz':
                        dates = pd.bdate_range(
                            start='1/1/2000', end='12/1/2022', freq='M'
                        )[::-1]
                        df = create_df(dates)
                        df.to_sql(table, self.conn, if_exists='append')
    ...
    def update_table(self, date: str, table: str):
        ...
        try:
            with self.conn:
                cur.execute(
                    f'INSERT INTO {table} (date, done, in_progress)'
...
class BackfillHandler:
    ...
    def next_date(self) -> str:
        ...
        cur.execute(
            f'SELECT MAX(date) FROM {self.tablename} WHERE'
            ' done = false AND'
            ' in_progress = false;'
        )
    ...
    def set_task_done(self, date):
        ...
        try:
            ...
            logger.error(
                f'❌ task for {date} on table {self.tablename} '
    ...
    def set_task_unfinished(self, date):
        ...
        self.conn.commit()
        logger.warning(
            f'❗ task for {date} on table {self.tablename} was left '
