...
def at_start(sender, **kwargs):
    ...
    with sender.app.connection() as conn:
        sender.app.send_task(
            'initialize_satellite_download_db', connection=conn, queue='downloader.extract'
        )
        sender.app.send_task(
            'create_copernicus_data_tables', connection=conn, queue='downloader.extract'
        )
