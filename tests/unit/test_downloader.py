...
from satellite_downloader.extract_reanalysis import _format_dates, BR_AREA, DATA_DIR
...
class TestExtractMethods(unittest.TestCase):
    def test_brazil_coordinates_to_copernicus_api(self):
        self.assertEquals(BR_AREA, {'N': 5.5, 'W': -74.0, 'S': -33.75, 'E': -32.25})
    
    def test_default_data_directory(self):
        self.assertEquals(DATA_DIR, Path().home / 'copernicus_data')
