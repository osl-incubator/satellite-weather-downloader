...
from satellite_weather_downloader.celery_app.celeryapp import app, update_task_delay
from satellite_weather_downloader.celery_app.backfill import BackfillDB, BackfillHandler
    ...
    match task:
        ...
        case 'fetch_foz_weather':
            ...
            last_day = first_monthday.replace(
                day=calendar.monthrange(first_monthday.year, 
                                        first_monthday.month)[1]
            ).strftime("%Y-%m-%d")
...
def fetch_weather(task: str, tablename: str):
    match task:
        case 'fetch_brasil_weather':
            ...
            if not date:
                logger.warning("None task was found to fetch."
...
@app.task(name='fetch_brasil_weather', )
